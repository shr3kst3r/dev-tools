"""Persisted window layout for azdo-watch.

The dashboard remembers how you left it — where the detail pane lives (`d`),
where the divider sits (`[` / `]`), whether the chart strip is shown (`g`), and
which project was selected — in a small JSON state file. Parsing/serializing is
pure so it can be unit-tested; only `load`/`save` touch the filesystem, and both
shrug off a missing, malformed, or unwritable file (a broken state file must never
take the dashboard down — it just means default layout).

Deliberately a mirror of `airflow_watch/layout.py` rather than an import of it:
`state_path()` there is hardcoded to `airflow-watch`, and the `Layout` fields
differ (a project key, not a deployment key). Generalising the two into one
module would mean changing a working, unrelated tool for a cosmetic win — the
same call `airflow_watch/layout.py` made about `my_prs/layout.py`, and the same
noted follow-up.
"""
...
class Layout:
    ...
    # The project key (its Azure DevOps GUID) that was selected last. Empty means
    # "no preference — take the first one".
    project: str = ""
...
def state_path() -> Path:
    ...
    return Path(config_home) / "azdo-watch" / "layout.json"
