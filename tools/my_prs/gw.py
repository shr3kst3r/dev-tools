"""Launching a goblin-watcher (gw) task from a PR.

`gw new --pr <url>` checks out the PR's head branch (plus its base) in a fresh
worktree and spawns an agent on it, resolving the project by matching the PR's
repo against gw's registered projects. The dashboard shells out to the gw
binary rather than importing goblin-watcher: gw owns its own config, state,
and tmux windowing, and its CLI is the stable seam between the two tools.
...
suspends itself around the call. stderr is captured to classify a failure:
the one the dashboard can resolve itself is the "task already exists"
collision, which it offers to retry with --rm.
"""
...
def new_pr_command(url: str, *, rm: bool = False) -> list[str]:
    """The `gw new` invocation for a PR. `--rm` removes an existing task with
    the same id first (gw keeps the PR's branch — only the stale worktree and
    task record go — and refuses if that worktree has uncommitted changes)."""
    cmd = ["gw", "new", "--pr", url]
    if rm:
        cmd.append("--rm")
    return cmd
...
def run_new(url: str, *, rm: bool = False) -> GwLaunch:
    """Run `gw new --pr` with the terminal's stdin/stdout (call this with the
    ...
    proc = subprocess.run(new_pr_command(url, rm=rm), stderr=subprocess.PIPE, text=True)
    if proc.stderr:
